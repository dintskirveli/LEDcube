...
Window::Window()
{
    ...
    xSpinbox->setValue(settings->value("xSize", 20).toInt());
    ...
    LEDStatus     = new QVBoxLayout;
    Status        = new QLabel(tr("LED Status"));
    onStautus     = new QCheckBox(tr("On"));

    onStautus->setChecked(false);
    LEDStatus->addWidget(Status);
    LEDStatus->addWidget(comboBox);
    LEDStatus->addWidget(drawOff);

    resolutionLayout->addLayout(LEDStatus);

    QVBoxLayout* labelLayout = new QVBoxLayout;
    QLabel* transparencyLabel = new QLabel(tr("Transparency"));
    QLabel* cubesizeLabel = new QLabel(tr("Cube Size   "));
    ...
    transparencySlider->setRange(0, 10);
